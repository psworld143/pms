-- Fix Schema Mismatch for Inventory Management System
-- Hotel PMS Training System - Inventory Module
-- This script fixes column name mismatches between code and database schema

-- Check if inventory_items table exists and fix column names
-- First, check if the table has 'name' column (old schema) or 'item_name' column (new schema)
-- Add item_name column if it doesn't exist
ALTER TABLE `inventory_items` 
ADD COLUMN IF NOT EXISTS `item_name` varchar(200) NOT NULL DEFAULT '' AFTER `id`;

-- Check if 'name' column exists and copy data to item_name
-- This will only work if the table was created with the old schema
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'inventory_items' 
     AND COLUMN_NAME = 'name') > 0,
    'UPDATE `inventory_items` SET `item_name` = `name` WHERE `item_name` = "" OR `item_name` IS NULL',
...
ALTER TABLE `inventory_items` 
ADD COLUMN IF NOT EXISTS `current_stock` int(11) NOT NULL DEFAULT 0 AFTER `quantity`;

-- Check if 'quantity' column exists and copy data to current_stock
SET @sql2 = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'inventory_items' 
     AND COLUMN_NAME = 'quantity') > 0,
    'UPDATE `inventory_items` SET `current_stock` = `quantity` WHERE `current_stock` = 0 AND `quantity` > 0',
    'SELECT "quantity column does not exist" as message'
));
PREPARE stmt2 FROM @sql2;
EXECUTE stmt2;
DEALLOCATE PREPARE stmt2;

-- Add is_pos_product column if it doesn't exist
ALTER TABLE `inventory_items` 
ADD COLUMN IF NOT EXISTS `is_pos_product` tinyint(1) NOT NULL DEFAULT 0 AFTER `status`;

-- Add last_updated column if it doesn't exist
ALTER TABLE `inventory_items` 
ADD COLUMN IF NOT EXISTS `last_updated` timestamp NULL DEFAULT NULL AFTER `updated_at`;

-- Update last_updated to current timestamp for existing records
UPDATE `inventory_items` 
SET `last_updated` = `updated_at` 
WHERE `last_updated` IS NULL;

-- Create inventory_requests table if it doesn't exist
...
-- Create reorder_rules table if it doesn't exist
...
-- Create floors table if it doesn't exist (for room inventory)
...
-- Create rooms table if it doesn't exist (for room inventory)
...
-- Insert sample data for floors and rooms if they don't exist
...
-- Insert sample rooms for each floor
...
-- Insert sample inventory categories if they don't exist
...
-- Update the item_name column to match the name column for existing records (if name column exists)
SET @sql3 = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'inventory_items' 
     AND COLUMN_NAME = 'name') > 0,
    'UPDATE `inventory_items` SET `item_name` = `name` WHERE `item_name` = "" OR `item_name` IS NULL',
    'SELECT "name column does not exist, skipping update" as message'
));
PREPARE stmt3 FROM @sql3;
EXECUTE stmt3;
DEALLOCATE PREPARE stmt3;

-- Update the current_stock column to match the quantity column for existing records (if quantity column exists)
SET @sql4 = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = DATABASE() 
     AND TABLE_NAME = 'inventory_items' 
     AND COLUMN_NAME = 'quantity') > 0,
    'UPDATE `inventory_items` SET `current_stock` = `quantity` WHERE `current_stock` = 0 AND `quantity` > 0',
    'SELECT "quantity column does not exist, skipping update" as message'
));
