-- Fix Room Inventory Tables - Working Version
...
-- Step 6: Add foreign key for assigned_housekeeping (only if it doesn't exist)
...
SET @constraint_exists = (
    ...
    AND CONSTRAINT_NAME = 'fk_rooms_assigned_housekeeping'
);

-- Only add the constraint if it doesn't exist
SET @sql = IF(@constraint_exists = 0, 
    'ALTER TABLE `rooms` ADD CONSTRAINT `fk_rooms_assigned_housekeeping` FOREIGN KEY (`assigned_housekeeping`) REFERENCES `users`(`id`) ON DELETE SET NULL',
...
-- Step 7: Insert sample inventory items (using only existing columns)
INSERT IGNORE INTO `inventory_items` (`item_name`, `description`, `current_stock`, `minimum_stock`, `unit_price`, `status`) VALUES
('Bath Towel', 'White cotton bath towel', 100, 20, 15.00, 'active'),
('Hand Towel', 'White cotton hand towel', 100, 20, 8.00, 'active'),
('Face Towel', 'White cotton face towel', 100, 20, 5.00, 'active'),
('Bath Soap', 'Luxury hotel bath soap', 200, 50, 3.00, 'active'),
('Shampoo', 'Hotel shampoo 50ml', 200, 50, 2.50, 'active'),
('Conditioner', 'Hotel conditioner 50ml', 200, 50, 2.50, 'active'),
('Hair Dryer', 'Professional hair dryer', 50, 10, 45.00, 'active'),
('Iron', 'Hotel iron with board', 30, 5, 25.00, 'active'),
('Coffee Maker', 'Single serve coffee maker', 25, 5, 80.00, 'active'),
('Television Remote', 'TV remote control', 50, 10, 15.00, 'active');

-- Step 8: Insert sample room inventory data
...
-- Step 9: Insert sample supply requests
...
-- Step 10: Assign some rooms to housekeeping user (assuming user_id = 1)
UPDATE `rooms` SET `assigned_housekeeping` = 1 WHERE `room_number` IN ('201', '202', '203', '204', '205');

-- Step 11: Create sample room inventory transactions
