...
try {
    $stmt = $pdo->prepare("SELECT * FROM customer_service_scenarios WHERE id = ?");
    ...
    if ($attempt_id) {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE id = ? AND user_id = ? AND scenario_id = ? AND scenario_type = \"customer_service\"
        ");
        $stmt->execute([$attempt_id, $user_id, $scenario_id]);
        $attempt = $stmt->fetch();
        
        if (!$attempt) {
            header("Location: customer-service-start.php?id=" . $scenario_id);
            exit();
        }
        ...
        $stmt = $pdo->prepare("
            ...
            VALUES (?, ?, \"customer_service\", \"in_progress\", NOW())
        ");
...
ini_set('log_errors', 1); echo $question["id"]; ?>">
                        <input type="hidden" name="scenario_type" value="customer_service">
...
ini_set('log_errors', 1); else: ?>
                    ...
                    <div class="text-center py-12">
                        ...
                        <a href="customer-service-results.php?attempt_id=<?php
// Error handling for production
ini_set('display_errors', 0);
ini_set('log_errors', 1); echo $attempt_id; ?>" 
...
ini_set('log_errors', 1); endif; ?>
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                        <strong>Situation:</strong> <?php
// Error handling for production
ini_set('display_errors', 0);
ini_set('log_errors', 1); echo htmlspecialchars($scenario["situation"]); ?>
                    </div>
                    <div>
                        <strong>Guest Request:</strong> <?php
// Error handling for production
ini_set('display_errors', 0);
ini_set('log_errors', 1); echo htmlspecialchars($scenario["guest_request"]); ?>
                    </div>
                </div>
            </div>
        </main>
