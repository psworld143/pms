...
try {
	...
	try {
		...
		// Ensure invoice is finalized to capture final charges on check-out
		try { ob_start(); require_once __DIR__ . '/finalize-invoice.php'; ob_end_clean(); } catch (Throwable $ignore) {}
		
		// If paid, mark formal bill as paid
		if (strtolower((string)$payment_status) === 'paid') {
			try {
				$updBill = $pdo->prepare("UPDATE bills SET status = 'paid', updated_at = NOW() WHERE reservation_id = ?");
				$updBill->execute([$reservation_id]);
			} catch (PDOException $e) { error_log('Bill status update warning: ' . $e->getMessage()); }
		}
