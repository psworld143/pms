...
function pmsErrorHandler($errno, $errstr, $errfile, $errline) {
    ...
    error_log("PMS Fatal Error - No redirect to 505.php: [$errno] $errstr in $errfile on line $errline");
    return false; // Let PHP handle the error normally
}
...
function pmsExceptionHandler($exception) {
    ...
    // Log exception instead of redirecting
    error_log("PMS Uncaught Exception - No redirect to 505.php: " . $exception->getMessage());
    return; // Let PHP handle the exception normally
}
...
register_shutdown_function(function() {
    ...
    if ($error !== null && in_array($error['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR, E_USER_ERROR])) {
        ...
        // Log fatal error instead of redirecting
        error_log("PMS Fatal Error - No redirect to 505.php: " . $error['message'] . " in " . $error['file'] . " on line " . $error['line']);
    }
});
...
function pmsCheckDatabaseConnection($conn) {
    if (!$conn) {
        error_log("PMS Database connection failed - No redirect to 505.php");
        return false;
    }
    return true;
}
...
function pmsCheckDatabaseQuery($result, $query_name = "Unknown") {
    if (!$result) {
        ...
        error_log("PMS Database error in $query_name query - No redirect to 505.php: $error_message");
        return false;
    }
    return true;
}
...
function pmsCheckDatabaseStatement($stmt, $query_name = "Unknown") {
    if (!mysqli_stmt_execute($stmt)) {
        global $conn;
        $error_message = mysqli_error($conn);
        error_log("PMS Database error in $query_name statement - No redirect to 505.php: $error_message");
        return false;
    }
    return true;
}
...
function pmsSafeInclude($file_path) {
    if (!file_exists($file_path)) {
        error_log("PMS Required file not found - No redirect to 505.php: $file_path");
        return false;
    }
    ...
    try {
        ...
        error_log("PMS Error including file $file_path - No redirect to 505.php: " . $e->getMessage());
        return false;
    }
    return true;
}
...
function pmsHandleMaintenanceMode() {
    if (pmsIsMaintenanceMode() && !pmsIsAdmin()) {
        error_log("PMS Maintenance mode active - No redirect to 505.php");
        return false;
    }
    return true;
}
...
function pmsHandle505Error() {
    error_log("PMS 505 error occurred - No redirect to 505.php");
    return false;
}
